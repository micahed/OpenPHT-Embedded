...
cleanup() {
  ...
  # wait for openpht to exit
  while killall -0 openpht.bin &>/dev/null; do
    sleep 0.5
  done
}
...
single_stacktrace()
{
  # core filename is "core.*openpht.bin.*"
  find "$1" -name 'core.*openpht.bin.*' | while read core; do
    echo "=====>  Core file: "$core"" >> $FILE
    echo "        =========================================" >> $FILE
    gdb /usr/lib/openpht/openpht.bin --core="$core" --batch -ex "thread apply all bt" 2>/dev/null >> $FILE
    rm -f "$core"
  done
}
...
activate_safe_mode()
{
  ...
  if [ "${BOOT_STATE:-OK}" = "OK" ]; then
    # generate logfiles zip for the failed openpht
    /usr/bin/createlog
...
print_crash_report()
{
  ...
  FILE="$CRASHLOG_DIR/.openpht_crashlog.log"
  echo "############## openpht CRASH LOG ###############" > $FILE
  echo >> $FILE
  echo "################ SYSTEM INFO ################" >> $FILE
  echo -n " Date: " >> $FILE
  date >> $FILE
  echo " openpht Options: $SAVED_ARGS" >> $FILE
  echo -n " Arch: " >> $FILE
  uname -m >> $FILE
  echo -n " Kernel: " >> $FILE
  uname -rvs >> $FILE
  ...
  echo "################# LOG FILE ##################" >> $FILE
  echo >> $FILE
  cat $KODI_ROOT/temp/openpht.log >> $FILE
  echo >> $FILE
  echo "############### END LOG FILE ################" >> $FILE
  echo >> $FILE
  echo "############ END openpht CRASH LOG #############" >> $FILE
  OFILE="$FILE"
  FILE="$CRASHLOG_DIR/openpht_crashlog_$DATE.log"
  mv "$OFILE" "$FILE"
  ln -sf "$(basename $FILE)" "$CRASHLOG_DIR/openpht_crash.log"
...
/usr/lib/openpht/openpht.bin $SAVED_ARGS
RET=$?
...
if [ $(( ($RET >= 131 && $RET <= 136) || $RET == 139 )) = "1" ] ; then
  ...
  rm -f $(ls -1t $CRASHLOG_DIR/openpht_crashlog_*.log | tail -n +11)
